...
def post_detail(request, pk):
    # 아래쪽, get() 코드를 get.object_or_404 문법으로 교체해주세요.
    post = get_object_or_404(Post, pk=pk)
    return render(request, 'blog/post_detail.html', {'post': post})

# post_new는 폼으로 연결해주는 ㅎ마수이다.
def post_new(request):
    '''
    #만들어놨던 폼 양식을 가져오기 위해서는 변수 = 폼양식()을 써야한다. ()가 우측에 붙음에 주의
    #현재코드는 PostForm() 양식을 따라 만들것임을 보여준다.
    form = PostForm()
    # 저장한 폼 양식은 템플릿 파일에 같이 넘겨야한다.
    return render(request, 'blog/post_edit.html', {'form': form})
    '''
    #request는 사용자의 요청, method는 요청양식
    ...
    if request.method == "POST":
        ...
        # 만들어놨던 폼양식을 가져오기 위해서는 변수 = 폼양식()를 써야한다. ()가 우측에 붙음에 주의
        # 현재코드는 PostForm() 양식을 따라 만들것임을 보여준다.
        form = PostForm()
